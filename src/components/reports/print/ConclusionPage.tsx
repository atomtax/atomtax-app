    ...
    <div style={a4LastPageStyle}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', marginBottom: '20px' }}>
        {keyMetrics.map(({ label, value, sub, bg }) => (
          <div key={label} style={{
            ...
            <p style={{ fontSize: '9px', color: 'rgba(255,255,255,0.85)', margin: '0 0 8px', fontWeight: 500 }}>{label}</p>
            <p style={{ fontSize: '14px', fontWeight: 700, color: 'white', margin: 0, whiteSpace: 'nowrap' }}>{value}</p>
            <p style={{ fontSize: '8px', color: 'rgba(255,255,255,0.7)', margin: '4px 0 0' }}>{sub}</p>
          </div>
        ))}
      </div>
      ...
      {isSincerefiling && (
        <div style={{
          ...
          <span style={{ fontSize: '9px', color: PRINT_TOKENS.textSecondary, fontWeight: 600 }}>
            성실신고 확인 대상
          </span>
        </div>
      )}
      ...
      {additionalNotes && additionalNotes.trim() && (
        <div style={{
          ...
          <p style={{ fontSize: '9px', fontWeight: 600, color: PRINT_TOKENS.textTertiary, margin: '0 0 6px', letterSpacing: '0.4px' }}>ADDITIONAL NOTES</p>
          <p style={{ fontSize: '10px', color: PRINT_TOKENS.textSecondary, margin: 0, lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>
            {additionalNotes}
          </p>
        </div>
      )}
      ...
      <div style={{
        ...
        <div>
          <p style={{ fontSize: '10px', fontWeight: 600, color: PRINT_TOKENS.primary, margin: '0 0 3px' }}>
            아톰세무회계 — {reportYear}년 법인세 신고 완료
          </p>
          <p style={{ fontSize: '9px', color: PRINT_TOKENS.textSecondary, margin: 0, lineHeight: 1.5 }}>
