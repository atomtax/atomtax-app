          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            {flowBoxes.map(({ label, value, bg, textColor }, i) => (
              <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '4px', flex: 1 }}>
                <div style={{
                  flex: 1, background: bg, borderRadius: '5px', padding: '10px 12px',
                  WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact',
                } as React.CSSProperties}>
                  <p style={{ fontSize: '12px', color: textColor === 'white' ? 'rgba(255,255,255,0.85)' : PRINT_TOKENS.textTertiary, margin: '0 0 4px', fontWeight: 500 }}>{label}</p>
                  <p style={{ fontSize: '14px', fontWeight: 600, color: textColor, margin: 0, whiteSpace: 'nowrap' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '4px' }}>
              ...
              <p style={{ fontSize: '13px', color: PRINT_TOKENS.primary, margin: 0, fontWeight: 500 }}>
                실효세율&nbsp;
                <span style={{ fontSize: '16px', fontWeight: 700 }}>{effectiveTaxRate}%</span>
              </p>
            </div>
            <p style={{ fontSize: '20px', fontWeight: 700, color: PRINT_TOKENS.primary, margin: 0, whiteSpace: 'nowrap' }}>
              {formatNumber(finalTax)}<span style={{ fontSize: '13px', fontWeight: 400, marginLeft: '3px' }}>원</span>
            </p>
            <p style={{ fontSize: '12px', color: PRINT_TOKENS.textSecondary, margin: '4px 0 0' }}>
              실효세율 = 최종세액 ÷ 법인세차감전이익
            </p>
          </div>
        </div>
