...
export default async function ClientDetailPage({ params }: Props) {
  ...
  const [client, taxReports, allInvoices] = await Promise.all([
    getClientById(id),
  ...
  const invoices = allInvoices.filter((inv) => inv.client_id === id)
            ...
            {invoices.map((invoice) => (
              ...
              >
                ...
                <div className="flex gap-6 text-gray-500">
                  ...
                  <span>청구금액 {formatCurrency(invoice.final_fee)}</span>
                </div>
                <Link
                  href={`/invoices/adjustment?id=${invoice.id}`}
                  className="text-xs text-indigo-600 hover:underline"
                >
                  보기
                </Link>
